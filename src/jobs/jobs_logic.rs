...
use crate::chains::chain_logic::get_token_decimals_by_chain_name;
        ...
        for action_node in o3.nodes {
            ...
            match action_node.clone() {
                MultiNodes::ChainTx(chainnode) => {
                    ...
                    process_chain_node(&mut chain_node, &webhook_loot);
                ...
                MultiNodes::Http(chain_node) => {
                    ...
                    process_node(&mut node_copy, &webhook_loot);
...
pub async fn dummy_thread(
    tx: Arc<tokio::sync::mpsc::Sender<Command>>,
